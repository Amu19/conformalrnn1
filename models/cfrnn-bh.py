...
# Extended by Sandeep Krishnan, 2022
...
class AuxiliaryForecaster(torch.nn.Module):
    ...
    uncertainty intervals are added in the main CFRNN-BH network.
    """
...
class CFRNN-BH:
    """
    The basic CFRNN-BH model as presented in Algorithm 1 of the accompanying paper.

    CFRNN-BH training procedure entails training the underlying (auxiliary) model
    ...
    def calibrate(self, calibration_dataset: torch.utils.data.Dataset):
        ...
        #kris corrected_q  = min((n_calibration + 1.0) * (1 - self.alpha / self.horizon) / n_calibration, 1)
       
        corrected_q1 = min(((n_calibration + 1.0) * (1 - self.alpha / self.horizon) / n_calibration)*(1 - self.fdr), 1)  #kris
        ...
        self.corrected_critical_calibration_scores = get_critical_scores(
            calibration_scores=self.calibration_scores, q=corrected_q1
        )
    ...
    def predict(self, x, state=None, corrected=True):
        ...
        Args:
            ...
            corrected: whether to use Benjamini Hochberg-corrected calibration scores
    ...
    def evaluate_coverage(self, test_dataset: torch.utils.data.Dataset, corrected=True):
        ...
        Args:
            ...
            corrected: whether to use the Benjamini Hochberg-corrected critical
            calibration scores
        Returns:
...
class AdaptiveCFRNN-BH(CFRNN-BH, torch.nn.Module):
    def __init__(
        self,
        embedding_size,
        input_size=1,
        ...
        **kwargs
        ...
        super(AdaptiveCFRNN-BH, self).__init__(
